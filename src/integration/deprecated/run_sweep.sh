#!/bin/bash
# Hyperparameter sweep script using SLURM for GPU acceleration
# Usage: 
#   sbatch run_sweep.sh                    # Full sweep
#   sbatch run_sweep.sh quick              # Quick test sweep

#SBATCH --job-name=fusion_sweep
#SBATCH --output=sweeps/%x_%j.out
#SBATCH --error=sweeps/%x_%j.err
#SBATCH --time=8:00:00                     # Longer time for full sweep
#SBATCH --mem=39G
...
export WANDB_GROUP="sweep_$(date +%Y%m%d_%H%M%S)"
...
# Note: SLURM doesn't pass command line args to the script
# Check if we should run quick test based on job name or environment variable
if [ "$SLURM_JOB_NAME" = "fusion_sweep_quick" ] || [ "$SWEEP_MODE" = "quick" ]; then
    echo "Running quick test sweep..."
    uv run python src/integration/hyperparameter_sweep.py quick
else
    echo "Running full hyperparameter sweep..."
    uv run python src/integration/hyperparameter_sweep.py
fi
